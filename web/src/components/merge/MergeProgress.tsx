...
const STEPS = [
  { label: "Fetching diff", min: 0, max: 40 },
  { label: "Building change summary", min: 40, max: 55 },
  { label: "Claude impact analysis", min: 55, max: 90 },
  { label: "Analysing merge risks", min: 78, max: 92 },
  { label: "Generating report", min: 90, max: 100 },
  { label: "Done", min: 100, max: 101 },
];
...
export function MergeProgress({ updates, pct, platform }: Props) {
  return (
    <div className="space-y-6">
      <ProgressFeed
        title={`Jessie Merge Review — ${platform.toUpperCase()}`}
        updates={updates}
        pct={pct}
      />
